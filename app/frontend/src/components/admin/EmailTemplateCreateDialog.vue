<template>
  <Dialog
    ...
    class="email-template-create-dialog"
    @hide="handleClose"
  >
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="field">
            ...
            <InputText
              ...
              placeholder="USER_ACCOUNT_CREATED"
              class="w-full"
...
import { EmailCategory, type CreateEmailTemplateDto, type TemplateVariable } from '@/types/email';
...
const formData = ref<CreateEmailTemplateDto>({
  name: '',
  displayName: '',
  description: '',
  subject: '',
...
const validateName = () => {
  ...
  if (!formData.value.name) {
    ...
    errors.value.name = 'Must be uppercase with underscores only (e.g., USER_ACCOUNT_CREATED)';
  } else {
...
const validateForm = (): boolean => {
  ...
  if (!formData.value.name) {
    errors.value.name = 'Template name is required';
  } else if (!/^[A-Z0-9_]+$/.test(formData.value.name)) {
    errors.value.name = 'Must be uppercase with underscores only';
  }
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const newTemplate = await createTemplate(formData.value);
...
/* Scrollbar styling */
.template-create-form::-webkit-scrollbar,
.variables-list::-webkit-scrollbar {
  width: 8px;
}

.template-create-form::-webkit-scrollbar-track,
.variables-list::-webkit-scrollbar-track {
  background: #f1f5f9;
  border-radius: 4px;
}

.template-create-form::-webkit-scrollbar-thumb,
.variables-list::-webkit-scrollbar-thumb {
  background: #cbd5e1;
  border-radius: 4px;
}

.template-create-form::-webkit-scrollbar-thumb:hover,
